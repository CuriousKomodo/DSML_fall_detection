...
class Composite_DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
            # load by batch.
        data = self.data_list[idx *self.batch_size:(idx + 1) * self.batch_size]
...
class Composite_LSTM_autoencoder(object):
    ...
    def build_model(self):
        ...
        if not self.read_out:
            future_decoder = CuDNNLSTM(self.latent_dim, return_sequences=True, kernel_regularizer=regularizers.l2(self.W_regularizer_val),
                    kernel_initializer=initializer)
            future_decoder_inputs = layers.Lambda(lambda x: x[:, :-1, :])(future_poses_DO)
            future_decoder_outputs = future_decoder(future_decoder_inputs, initial_state=encoder_states)
            future_ts_dense_inputs = Activation('relu')(future_decoder_outputs)
            future_ts_dense = layers.TimeDistributed(layers.Dense(self.input_dim, kernel_initializer=initializer))
            future_pred = future_ts_dense(future_ts_dense_inputs)
            future_pred = Activation('relu')(future_pred)
            ...
            if not self.residual:
                future_decoder = RecurrentSequential(readout='readout_only',return_sequences=True,
                    teacher_force=self.teacher_force) # previous output will be added to input
                ...
                future_pred = future_decoder(future_decoder_inputs,initial_state = encoder_states)#ground_truth = future_ground_truth

            else:#train residue
                print('Residual architecture!!!!!!!!!!!!!!!')
                x = Input((self.input_dim,))
                h_tm1 = Input((self.latent_dim,))
                c_tm1 = Input((self.latent_dim,))
                readout_input = Input((self.input_dim,))

                lstms_input = readout_input
                lstms_output, h, c = lstms_input, h_tm1, c_tm1
                lstms_output, h, c = LSTMCell(self.latent_dim)([lstms_output, h, c])
                ...
                future_pred = future_decoder(future_decoder_inputs,initial_state = encoder_states,
...
result_path = '/Users/kefei/Documents/mm_fall_detection/results/win%s/composite'%window
model_path = '/Users/kefei/Documents/mm_fall_detection/models/win%s/composite'%window
