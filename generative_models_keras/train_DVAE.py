...
hidden_dims_pair = [[64,128],[64,256],[128,256]]
latent_dims = [20,40,60]

repeats = 2
val_losses = np.zeros((repeats, len(hidden_dims_pair),len(latent_dims)))

for r in range(repeats):
    train_valid_split(keep_3D=False)
    print('finish spliting')
    target_valid = np.load('/Users/kefei/Documents/Dataset/NTU/single_poses/integrated/2D_DAE/X_train_valid.npy')
    target_valid = standardize_data(target_valid,time_sequence=False)
    print(target_valid.shape)
    input_valid = remove_joints(target_valid, max_num_remove=5,num_joints=17,dim=2)
    print('error due to missing joint = ',mse(input_valid, target_valid, axis=None))
    #if r==0: #check the generation of data.
    #    np.save('/Users/kefei/Documents/Dataset/NTU/single_poses/integrated/2D/dropout_train_valid.npy', input_valid)

    for i in range(len(hidden_dims_pair)):
        for j in range(len(latent_dims)):
            hidden_dim1 = hidden_dims_pair[i][0]
            hidden_dim2 = hidden_dims_pair[i][1]
            latent_dim = latent_dims[j]
            print('hidden_dim1=',hidden_dim1)
            print('hidden_dim2=',hidden_dim2)
            print('latent_dim=',latent_dim)
            optimizer=optimizers.Adam(0.001) #the literature suggested 10**(-3)
            model_params = {'hidden_dim1': hidden_dim1, 'hidden_dim2': hidden_dim2, 'latent_dim':latent_dim,'input_dim':34}
            print(model_params)
            dvae = DVAE(**model_params)
            model = dvae.model
            model.compile(optimizer=optimizer, loss=dvae.vae_loss)
            ...
            train = model.fit_generator(generator=DAE_generator(), epochs=15,validation_data=(input_valid, target_valid),
                                callbacks=[keras.callbacks.EarlyStopping(patience=2),reduce_lr])
...
np.save('/Users/kefei/Documents/mm_fall_detection/demo/results/2D/DAE/DVAE_CV_losses.npy',val_losses)
