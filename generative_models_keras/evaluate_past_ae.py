...
def fitness(hyperparams, classifier_bool, result_path, model_path): #annealtime
    ...
    file_path = '/Users/kefei/Documents/results/win60/past/AE/with_classifier/call_back_result_with_classifier.hdf5'
    ...
    params = {'hidden_dim': hidden_dim,'latent_dim': latent_dim,'W_regularizer_val': hidden_W_regularizer,
        ...
        'VAE':False, 'input_dim':51, 'joint_dropout':False,
    ...
    if classifier_bool:
        ...
        past_ae.model.compile(optimizer,loss={'reconstructed_poses':'mean_squared_error'},
    ...
    '''
    h_past = past_ae.enc_model.predict(X_train)
    np.save(result_path+'hidden_train.npy',h_past)

    h_past = past_ae.enc_model.predict(X_test)
    np.save(result_path+'hidden_test.npy',h_past)
    recon = past_ae.recon_model.predict(X_test)

    #save reconstruction_loss
    MSE_TS = np.mean(np.square(X_test[:,::-1,:]-recon), axis=(0,2))
    np.save(result_path + 'recon_loss_overtime.npy', MSE_TS)

    recon_non_fall = recon[y_test==0][:100]
    recon_fall = recon[y_test==0][:100]
    np.save(result_path + 'recon_non_fall.npy', recon_non_fall)
    np.save(result_path + 'recon_fall.npy', recon_fall)

    if classifier_bool:
        pred = past_ae.classifier_model.predict(X_test)
...
def collect_result(classifier_bool):
    if classifier_bool:
        hyperparameters =[512, 128, 0.001, 0.01, 64, 0.0005]
        model_path = '/Users/kefei/Documents/results/win60/past/AE/with_classifier/'
        result_path = '/Users/kefei/Documents/mm_fall_detection/models/win60/past/AE/with_classifier/'

    fitness(hyperparameters, classifier_bool, result_path, model_path)

#already know for klanneal with classifier, just without reconstructed poses

collect_result(classifier_bool=True)
